...
contract PylonToken is owned {
    ...
    address payable public beneficiary = 0xAE0151Ca8C9b6A1A7B50Ce80Bf7436400E22b535;  //Chip-chap Klenergy Address of ether beneficiary account
    ...
    ///@notice emits FrozenFunds
    ///@notice emits TokenPurchase
    function buyTokens(address investor) public payable notPaused {
        require (!crowdsaleClosed); // Check if crowdsale is open or not
        require(investor != address(0));  // Check the address
    ...
    function buyIco(address investor) internal{
      ...
      require(balanceOf[address(this)] >= tokens);             // checks if it has enough to sell
      ...
      emit FrozenFunds(investor, true);

      emit TokenPurchase(msg.sender, investor, weiAmount, tokens);
    }
     ...
     */
     ///@notice emits GoalReached
    function checkGoalReached() public afterDeadline onlyOwner {
