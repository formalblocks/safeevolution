...
contract PylonToken is Ownable {
  ...
  function approveAndCall(address _spender, uint256 _value, bytes memory _extraData) public
    ...
    /// @notice  postcondition forall (address addr) (addr == _from || addr == _to || __verifier_old_uint(balances[addr]) == balances[addr]) || (__verifier_old_uint(balances[addr]) == balances[addr]) && !success
    /// @notice  postcondition _value >= 0
    /// @notice  emits  Transfer  
