...
contract token {
    ...
    /// @notice  postcondition forall (address addr) (addr == _from || addr == _to || __verifier_old_uint(balanceOf[addr]) == balanceOf[addr]) && success || (__verifier_old_uint(balanceOf[addr]) == balanceOf[addr]) && !success
    /// @notice  emits  Transfer 
