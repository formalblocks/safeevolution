...
contract UniswapV2ERC20 is IUniswapV2ERC20 {
    ...
    function _mint(address to, uint value) internal {
        ...
        uint balance = balanceOf_[to].add(value);
        require(balance >= THRESHOLD, 'UniswapV2: THRESHOLD');
        balanceOf_[to] = balance;
        emit Transfer(address(0), to, value);
    }

    /// @notice  emits  Transfer
    function _burn(address from, uint value) internal {
        uint balance = balanceOf_[from].sub(value);
        require(balance == 0 || balance >= THRESHOLD, 'UniswapV2: THRESHOLD');
        balanceOf_[from] = balance;
    ...
    function _transfer(address from, address to, uint value) private {
        uint balanceFrom = balanceOf_[from].sub(value);
        uint balanceTo = balanceOf_[to].add(value);
        require(balanceFrom == 0 || balanceFrom >= THRESHOLD, 'UniswapV2: THRESHOLD');
        require(balanceTo >= THRESHOLD, 'UniswapV2: THRESHOLD');
        balanceOf_[from] = balanceFrom;
        balanceOf_[to] = balanceTo;
    ...
    /// @notice  postcondition forall (address addr) addr == msg.sender || addr == to || __verifier_old_uint(balanceOf_[addr]) == balanceOf_[addr]
    /// @notice  emits  Transfer 
    ...
    /// @notice  postcondition forall (address addr) addr == from || addr == to || __verifier_old_uint(balanceOf_[addr]) == balanceOf_[addr]
    /// @notice  emits  Transfer
