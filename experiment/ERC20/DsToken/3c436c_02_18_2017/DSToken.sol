...
contract DSToken is IERC20 {
    ...
    /// @notice  postcondition ( ( _balances[src] ==  __verifier_old_uint (_balances[src] ) - x  &&  src  != dst ) ||   ( _balances[src] ==  __verifier_old_uint ( _balances[src] ) &&  src== dst ) &&  ok )   || !ok
    ...
    /// @notice  postcondition (_approvals[msg.sender ][ spender] ==  x  &&  ok) || ( _approvals[msg.sender ][ spender] ==  __verifier_old_uint ( _approvals[msg.sender ][ spender] ) && !ok )    
    /// @notice  emits  Approval
    function approve(address spender, uint x) public returns (bool ok) {
