...
contract SkinCoin is ERC20 {
  ...
  */
  ...
  /// @notice  postcondition forall (address addr) (addr == msg.sender || addr == _to || __verifier_old_uint(balances[addr]) == balances[addr]) && success || (__verifier_old_uint(_balances[addr]) == _balances[addr]) && !success
  /// @notice  emits  Transfer 
  ...
  /// @notice  postcondition  (allowed[_from ][msg.sender] ==  __verifier_old_uint (allowed[_from ][msg.sender] ) - _value && sucess) || (allowed[_from ][msg.sender] ==  __verifier_old_uint (allowed[_from ][msg.sender] ) && !sucess) || _from  == msg.sender
